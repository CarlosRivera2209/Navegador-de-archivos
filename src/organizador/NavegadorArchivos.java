...
public class NavegadorArchivos extends JFrame {
    ...
    if (dir != null && dir.isDirectory()) {
        ...
        if (archivos != null) {
            ...
            switch (criterio) {
                case "Nombre":
                    // Ordenar por nombre, primero carpetas y luego archivos, ambos en orden alfabético
                    comparator = Comparator.comparing(File::isFile)
                            .thenComparing(File::getName, String.CASE_INSENSITIVE_ORDER);
                    break;
                case "Fecha":
                    // Ordenar por fecha, del modificado más reciente al más antiguo
                    comparator = Comparator.comparingLong(File::lastModified).reversed()
                            .thenComparing(File::isFile);
                    break;
                case "Tipo":
                    // Ordenar por tipo (extensión), primero carpetas y luego archivos
                    comparator = Comparator.comparing((File f) -> f.isDirectory() ? "" : getFileExtension(f))
                            .thenComparing(File::getName, String.CASE_INSENSITIVE_ORDER);
                    break;
                case "Tamaño":
                    // Ordenar por tamaño, del más pesado al más ligero
            ...
            // Eliminar y añadir los nodos ordenados al árbol
